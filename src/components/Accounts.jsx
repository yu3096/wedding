...
const THEMES = [
  { // 신랑측 (파스텔 블루)
    ...
    text: "text-blue-900",
    btn: "bg-blue-900/10 hover:bg-blue-900/20 text-blue-900",
    closeBtn: "border-blue-900/20 bg-blue-900/5 hover:bg-blue-900/10 text-blue-900",
  ...
  { // 신부측 (파스텔 핑크)
    ...
    text: "text-rose-900",
    btn: "bg-rose-900/10 hover:bg-rose-900/20 text-rose-900",
    closeBtn: "border-rose-900/20 bg-rose-900/5 hover:bg-rose-900/10 text-rose-900",
              ...
              >
                <div
                  className={`h-16 sm:h-18 flex items-center justify-between px-4 sm:px-5
                              ${t.text} font-medium bg-gradient-to-r ${t.grad}`}
                >
                  <span className="text-lg sm:text-xl">{g.side}</span>
                  <button
                    onClick={() => openModal(i)}
                    className={`h-9 px-3 rounded-full ${t.btn}
                               text-sm transition outline-none ${t.focus}`}
        ...
        {modal.open && current && (
          <ModalPortal>
            ...
            >
              ...
              >
                <div className={`px-5 py-4 border-b ${theme.text} rounded-t-2xl bg-gradient-to-r ${theme.grad}`}>
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <button
                      onClick={beginClose}
                      className={`shrink-0 h-8 w-8 rounded-full border transition ${theme.closeBtn}`}
                      aria-label="닫기"
                      title="닫기"
                    >
                      ✕
